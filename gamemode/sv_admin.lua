...
function concommand.AddGM( cmd, func )
	
	local function c( ply, _, args )
		
		if( ply:EntIndex() != 0 and !ply:IsEventCoordinator() ) then
...
local function GiveMoney( ply, args )
	...
	if( targ and targ:IsValid() and amt > 0 ) then
		...
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " gave " .. targ:RPName() .. " $" .. tostring( amt ) .. ".", ply );
...
local function SetSalaryAmount( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		targ:SetSalaryAmount(amt)
		targ:UpdateCharacterField( "SalaryAmount", tostring( targ:SalaryAmount() ) );
		
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " set " .. targ:RPName() .. "'s salary amount to " .. tostring( amt ) .. ".", ply );
...
concommand.AddAdmin( "rpa_setsalaryamount", SetSalaryAmount );

local function SetSalaryInterval( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		targ:SetSalaryInterval(amt)
		targ:UpdateCharacterField( "SalaryInterval", tostring( targ:SalaryInterval() ) );
		
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " set " .. targ:RPName() .. "'s salary interval to " .. tostring( amt ) .. ".", ply );
...
concommand.AddAdmin( "rpa_setsalaryinterval", SetSalaryInterval );
...
concommand.AddGM( "rpg_goto", Goto );
...
concommand.AddGM( "rpg_bring", Bring );
...
local function Send( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		if( destination and destination:IsValid() ) then
			...
			net.Start( "nANoSecondTargetFound" );
			net.Send( ply );
...
concommand.AddGM( "rpg_send", Send );
...
concommand.AddGM( "rpg_seeall", Seeall );
...
concommand.AddGM( "rpg_setcharmodel", SetCharModel );
...
concommand.AddGM( "rpg_setname", SetName );
...
local function SetCombineFlag( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		local flagStruct = GAMEMODE:LookupCombineFlag( flag );
		...
		GAMEMODE:LogAdmin( "[F] " .. ply:Nick() .. " changed player " .. targ:RPName() .. "'s police flag to \"" .. flag .. "\".", ply );
...
local function SetCharFlag( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		for k,v in next, GAMEMODE.CharFlags do
		
			if( v.Flag == flag ) then
			
				if( !v.OnGiven ) then break end
...
local function GiveCharFlag( ply, args )
	...
	if( targ and targ:IsValid() and flag and #flag > 0 ) then
		
		if( targ:HasCharFlag( flag ) ) then return end
		
		targ:AddCharFlag( flag );
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " gave " .. targ:RPName() .. " " .. flag .. " flags.", ply );
...
concommand.AddAdmin( "rpa_givecharflag", GiveCharFlag );

local function TakeCharFlag( ply, args )
	...
	if( targ and targ:IsValid() and flag and #flag > 0 ) then
		...
		targ:TakeCharFlag( flag );
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " took " .. targ:RPName() .. "'s " .. flag .. " flags.", ply );
...
concommand.AddAdmin( "rpa_takecharflag", TakeCharFlag );
...
local function SetCombineSquad( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		GAMEMODE:LogAdmin( "[F] " .. ply:Nick() .. " changed player " .. targ:RPName() .. "'s police squad to \"" .. squad .. "\".", ply );
...
local function SetCombineSquadID( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		GAMEMODE:LogAdmin( "[F] " .. ply:Nick() .. " changed player " .. targ:RPName() .. "'s police squad ID to \"" .. squadid .. "\".", ply );
...
local function EditInventory( ply, args )
	...
	if( targ and targ:IsValid() ) then
		...
		net.Start( "nAEditInventory" );
			net.WriteEntity( targ );
			net.WriteTable( targ.Inventory[targ:GetDutyInventory()] );
		net.Send( ply );
...
function nARemoveItem( len, ply )
	...
	net.Start( "nARemoveItem" );
		net.WriteEntity( ply );
		net.WriteString( targ.Inventory[targ:GetDutyInventory()][k] );
	net.Send( targ );
	
	GAMEMODE:LogAdmin( "[I] " .. ply:Nick() .. " removed character " .. targ:RPName() .. "'s item \"" .. targ.Inventory[targ:GetDutyInventory()][k] .. "\".", ply );
	
	if( targ.Inventory[k] ) then
		
		GAMEMODE:GetItemByID( targ.Inventory[targ:GetDutyInventory()][k] ).OnRemoved( targ.Inventory[targ:GetDutyInventory()][k], targ );
		GAMEMODE:LogItems( "[R] " .. targ:VisibleRPName() .. "'s item " .. targ.Inventory[targ:GetDutyInventory()][k] .. " was removed by " .. ply:Nick() .. ".", ply );
		table.remove( targ.Inventory[targ:GetDutyInventory()], k );
...
concommand.AddGM( "rpg_togglesaved", ToggleSaved );
...
concommand.AddAdmin( "rpa_hideadmin", Hidden );
...
local function TakeFromStockpile( ply, args )
	...
	local t = args[2] and tonumber( args[2] ) or 1;
	
	if( targ ) then
	
		local item = targ.Stockpile[t];
		table.remove( targ.Stockpile, t );
		...
		GAMEMODE:LogAdmin( "[F] " .. ply:Nick() .. " removed item from stash \"" .. targ:RPName() .. "\".", ply );
...
local function SetEntityDesc( ply, cmd, args, szArgs )
	...
	if( !ply:IsAdmin() and !ply:IsEventCoordinator() ) then
		...
		if( targ and IsValid( targ ) and targ:GetClass() == "prop_physics" or targ:GetClass() == "prop_ragdoll" ) then
			...
			if( targ.PropSteamID and targ:PropSteamID() == ply:SteamID() ) then
...
local function AddFlag( ply, args )
...
concommand.AddAdmin( "rpa_giveflag", AddFlag );

local function TakeFlag( ply, args )
...
concommand.AddAdmin( "rpa_takeflag", TakeFlag );

local function SetFlags( ply, args )
	...
	if( targ and targ:IsValid() and #flags > 0 ) then
		
		targ:SetFlags( flags );
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " set " .. targ:RPName() .. "'s flags to " .. flags .. ".", ply );
...
concommand.AddAdmin( "rpa_setflags", SetFlags );

local function ToggleCanFire( ply, args )
	...
	if( targ and targ:IsValid() and GAMEMODE:LookupCombineFlag( targ:ActiveFlag() ) ) then
		
		targ:SetCanFireWeapon( !targ:CanFireWeapon() );
		GAMEMODE:LogAdmin( "[M] " .. ply:Nick() .. " toggled " .. targ:RPName() .. "'s CanFireWeapon state to " .. tostring(targ:CanFireWeapon()) .. ".", ply );
...
concommand.AddAdmin( "rpa_togglecanfire", ToggleCanFire );