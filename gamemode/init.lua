...
include( "sv_stockpiles.lua" );
include( "sv_augments.lua" );
include( "sv_stims.lua" );
...
AddCSLuaFile( "sh_augments.lua" );
AddCSLuaFile( "sh_stims.lua" );
...
AddCSLuaFile( "cl_stockpiles.lua" );
AddCSLuaFile( "cl_augments.lua" );
AddCSLuaFile( "cl_stims.lua" );
...
util.AddNetworkString( "nPopulateAugmentRemove" );

local function nPopulateAugmentRemove( len, ply )

	local target = net.ReadEntity();

	if( !target or !target:IsValid() ) then return end
	if( !ply:HasCharFlag( "Q" ) ) then return end
	if( ply:GetPos():Distance( target:GetPos() ) > 100 ) then return end
	
	net.Start( "nPopulateAugmentRemove" );
		net.WriteString( util.TableToJSON( target.Augments ) );
	net.Send( ply );
