...
function GM:CreateTeams()
	...
	team.SetUp( TEAM_CITIZEN, "Civilians", Color( 0, 120, 0, 255 ), false );
	team.SetUp( TEAM_COMBINE, "SDPD-ARCS", Color( 33, 106, 196, 255 ), false );
	team.SetUp( TEAM_OFFCOMBINE, "Off-Duty Police", Color( 0, 120, 0, 255 ), false );
	...
	team.SetUp( TEAM_OVERWATCH, "ASWAT AutoCops", Color( 200, 200, 200, 255 ), false );
...
function DoDistanceCheckForStockpile( ply )
...
function GM:CheckCharacterValidity( name, desc, titleone, titletwo, model, sum, traits )
	...
	if( sum < 0 or sum > GAMEMODE.StatsAvailable + 35 ) then
		return false, "Too many stats allocated.";
	end
	
	local traitssum = 0;
	for k,v in next, traits do
		traitssum = traitssum + GAMEMODE.Traits[k][3];
	end
	if( traitssum > 8 ) then
...
GM.BusinessLicenses[LICENSE_FFL] = { "FFL Dealer", 10000 };
GM.BusinessLicenses[LICENSE_AUG] = { "Augment Retailer", 8000 };
GM.BusinessLicenses[LICENSE_MED] = { "Medical", 2500 };

GM.TraitPoints = 8;
GM.Traits = { };
GM.Traits[TRAIT_NONE] = { "None", "No trait.", 0 };
GM.Traits[TRAIT_LOYALIST] = { "Radio Operator", "Spawn with a radio.", 2 };
GM.Traits[TRAIT_STRONG] = { "Strong", "Naturally strong, +2 Strength.", 6, {
	[TRAIT_VTHINBONED] = true,
	[TRAIT_VERYWEAK] = true,
	[TRAIT_THINBONED] = true,
}, {["Strength"] = 2} };
GM.Traits[TRAIT_LUCKY] = { "Lucky", "Lady luck smiles upon you.", 4, {[TRAIT_UNLUCKY] = true}, {["Luck"] = 4} };
GM.Traits[TRAIT_UNLUCKY] = { "Unlucky", "Lady luck has abandoned you.", -4, {[TRAIT_LUCKY] = true}, {["Luck"] = -4} };
GM.Traits[TRAIT_MULE] = { "Mule", "Strong back muscles allow you to carry heavier loads.", 4, {[TRAIT_VERYWEAK] = true}};
GM.Traits[TRAIT_SPY] = { "Spy", "Ability to conceal the lightest firearm from searches.", 6 };
GM.Traits[TRAIT_SPEEDY] = { "Dexterous", "Pat people down in half the time.", 2, { [TRAIT_THUMBS] = true } };
GM.Traits[TRAIT_THUMBS] = { "All Thumbs", "Pat people down in twice the time.", -2, { [TRAIT_SPEEDY] = true } };
GM.Traits[TRAIT_GREASE] = { "Thick Skinned", "You scrapped alot as a kid, and thus you've grown to be quite resilient. +2 Endurance.", 6, {}, 
{["Endurance"] = 2} };
GM.Traits[TRAIT_RUSSIAN] = { "Russian", "Can speak Russian with /rus.", 2 };
GM.Traits[TRAIT_CHINESE] = { "Chinese", "Can speak Chinese with /chi.", 2 };
GM.Traits[TRAIT_JAPANESE] = { "Japanese", "Can speak Japanese with /jap.", 2 };
GM.Traits[TRAIT_SPANISH] = { "Spanish", "Can speak Spanish with /spa.", 2 };
GM.Traits[TRAIT_FRENCH] = { "French", "Can speak French with /fre.", 2 };
GM.Traits[TRAIT_GERMAN] = { "German", "Can speak German with /ger.", 2 };
GM.Traits[TRAIT_ITALIAN] = { "Italian", "Can speak Italian with /ita.", 2 };
GM.Traits[TRAIT_THINBONED] = { "Underweight", "-2 Strength, -2 Endurance, higher chance of breaking bones.", -6, {
	[TRAIT_VTHINBONED] = true,
	[TRAIT_STRONG] = true,
}, {["Strength"] = -2, ["Endurance"] = -2} };
GM.Traits[TRAIT_VTHINBONED] = { "Very Underweight", "-4 Strength, -4 Endurance, very high chance of breaking bones.", -8, {
	[TRAIT_THINBONED] = true,
	[TRAIT_STRONG] = true,
}, {["Strength"] = -4, ["Endurance"] = -4} };
GM.Traits[TRAIT_VERYWEAK] = { "Weak", "-4 Strength, reduced carrying weight.", -10, { 
	[TRAIT_STRONG] = true, 
	[TRAIT_MULE] = true, 
}, {["Strength"] = -4} };
GM.Traits[TRAIT_FOREIGNER] = { "Foreigner", "You don't speak the local language.", -4 };
GM.Traits[TRAIT_MULTILINGUAL] = { "Multilingual", "You can speak more than two languages.", 4 };
GM.Traits[TRAIT_RUNNER] = { "Runner", "You're adept at running, enabling you to run faster and longer than normal. +2 Agility", 4, {
	[TRAIT_TWOFEET] = true,
}, {["Agility"] = 2} };
GM.Traits[TRAIT_TWOFEET] = { "Asthmatic", "You are unable to run as fast or as far as normal. -2 Agility", -4, {
	[TRAIT_RUNNER] = true,
}, {["Agility"] = -2} };
GM.Traits[TRAIT_NOCORTICAL] = {
	"Au Naturale",
	"You do not have a cortical stack installed. You are vulnerable to permanent death.",
	-4,
};
GM.Traits[TRAIT_VETERAN] = {
	"Veteran",
	"You served in the military, where you were outfitted with a stimulant diffusor for military-grade stims.",
	4,
};
GM.Traits[TRAIT_INVESTOR] = { "Investor", "An wise investment earned you a little extra spending money.", 4 };
...
if (SERVER and game.IsDedicated()) then
	...
	local function ArmDupe( len, ply )

		MsgC( Color(255,0,0), Format("Player %s is calling ArmDupe! May be attempting to crash the server.\nIf you see this message repeatedly, it is an attempt to crash the server.\n", ply:Name()) );
		local message = ply:Name().." ["..ply:SteamID().."] has possibly attempted to crash the server with 'ArmDupe'"
