...
function nCPatDownStart( len, ply )
	...
	if( ply:HasTrait( TRAIT_SPEEDY ) ) then
		
		mul = 0.5;
		
	elseif( ply:HasTrait( TRAIT_THUMBS ) ) then
...
function nCPatDown( len, ply )
	...
	if( targ:GetVelocity():Length2D() <= 5 ) then
		...
		for k, v in pairs( targ.Inventory[ply:GetDutyInventory()] ) do
			
			if( ( GAMEMODE:GetItemByID(v).Weapon and GAMEMODE:GetItemByID(v).Weight < 3 ) and targ:HasTrait( TRAIT_SPY ) ) then
