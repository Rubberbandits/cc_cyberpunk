...
language.Add( "npc_combine_camera", "Police Camera" );
...
function GM:EndIntroCam()

	self.IntroCamStart = nil;
...
function nIntroStart()
	...
	if cookie.GetNumber( "cyberp_doneintro", 0 ) == 2 then
...
function GM:CalcView( ply, pos, ang, fov, znear, zfar )
	...
	if( self.IntroCamStart and self.IntroCamData ) then
		...
		if( CurTime() - self.IntroCamStart < self.IntroCamDelay * #self.IntroCamText ) then
			...
			if stage == #self.IntroCamText then
				self.IntroCamStart = CurTime()
			end
...
function GM:DrawHealthBars()
	...
	local b = 15 - ( LocalPlayer():Endurance() / 100 * 15 );
...
function GM:DrawEntities()
	...
	for _, v in pairs( player.GetAll() ) do
		...
		if( v != LocalPlayer() or LocalPlayer():GetViewEntity() != LocalPlayer() ) then
			...
			if( v:Ragdoll() and v:Ragdoll():IsValid() ) then
				...
				if( ( self.SeeAll or ( pos.visible and LocalPlayer():CanSee( v:Ragdoll() ) and LocalPlayer():GetPos():Distance( v:GetPos() ) < self:GetPlayerSight() ) and LocalPlayer():GetEyeTraceNoCursor().Entity == v ) and v:Alive() ) then
				...
				if( ( self.SeeAll or ( pos.visible and LocalPlayer():CanSee( v ) and LocalPlayer():GetPos():Distance( v:GetPos() ) < self:GetPlayerSight() and !v:GetNoDraw() and LocalPlayer():GetEyeTraceNoCursor().Entity == v ) ) and v:Alive() ) then
			...
			if( v.TitleAlpha > 0 ) then
				...
				if #v:DescriptionModifier() > 0 then
					draw.DrawTextShadow( v:DescriptionModifier(), "CombineControl.LabelSmall", pos.x, pos.y, Color( 200, 200, 200, v.TitleAlpha * 255 ), Color( 0, 0, 0, v.TitleAlpha * 255 ), 1 );
					pos.y = pos.y + 20;
				end
	...
	for _, v in next, ents.FindInSphere(LocalPlayer():GetPos(), 300) do
	...
	for _, v in pairs( game.GetDoors() ) do
		...
		if( v.HUDAlpha > 0 ) then
			...
			if( ( v:DoorType() == DOOR_BUYABLE or v:DoorType() == DOOR_BUYABLE_ASSIGNABLE ) and #v:DoorOwners() == 0 and #v:DoorAssignedOwners() == 0 ) then
				...
				draw.DrawTextShadow("$" .. v:DoorPrice(), "CombineControl.PlayerFont", pos.x, pos.y, Color( 226, 205, 95, v.HUDAlpha * 255 ), Color( 0, 0, 0, v.HUDAlpha * 255 ), 1 );
				pos.y = pos.y + 20;
...
function GM:DrawUnconnected()
	...
	if( !self.CharCreateOpened and !self:InIntroCam() ) then
