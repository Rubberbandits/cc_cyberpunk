...
function nCReceiveCredits( len )
	...
	GAMEMODE:AddChat( Color( 200, 200, 200, 255 ), "CombineControl.ChatNormal", ply:VisibleRPName() .. " gave you $" .. amt .. ".", { CB_ALL, CB_OOC } );
	...
	if( ent and ent:IsValid() ) then
		...
		if( ent:IsDoor() ) then
			...
			elseif( ( ent:DoorType() == DOOR_BUYABLE or ent:DoorType() == DOOR_BUYABLE_ASSIGNABLE ) and table.HasValue( ent:DoorOwners(), LocalPlayer():CharID() ) ) then
				...
				local option = { "Sell", function()
					...
					self:AddChat( Color( 229, 201, 98, 255 ), "CombineControl.ChatNormal", "You sold the door for 80% of its original value ($" .. tostring( math.floor( ent:DoorPrice() * 0.8 ) ) .. ").", { CB_ALL, CB_IC } );
		...
		elseif( ent:IsPlayer() ) then
			...
			local option = { "Pat Down", function()
				...
				if( LocalPlayer():HasTrait( TRAIT_SPEEDY ) ) then
					
					mul = 0.5;
					
				elseif( LocalPlayer():HasTrait( TRAIT_THUMBS ) ) then
			...
			if( LocalPlayer():HasCharFlag( "Q" ) ) then
			
				local found = false;
				for k,v in next, ents.FindInSphere( LocalPlayer():GetPos(), 150 ) do
				...
				if( found ) then
					...
					local option = { "Install Augment", function()
					
						GAMEMODE:OpenAugmentInstall( ent );
						
					end, nil, 100 };
					
					table.insert( tab, option );
					
					local option = { "Remove Augment", function()
					
						GAMEMODE:OpenAugmentRemove( ent );
			...
			if( LocalPlayer():CombineFlag() == "F" and LocalPlayer():ActiveFlag() != "" ) then
			
				if( ent:HasAnyCombineFlag() and ent:ActiveFlag() != "" and ent:CombineSquad() == "" ) then
				
					local option = { "Add to Squad", function()
						
						net.Start( "nSetSquad" );
							net.WriteEntity( ent );
							net.WriteString( LocalPlayer():CombineSquad() );
						net.SendToServer();
				...
				if( ent:HasAnyCombineFlag() and ent:ActiveFlag() != "" and ent:CombineSquad() == LocalPlayer():CombineSquad() ) then
				
					local option = { "Remove from Squad", function()
...
function GM:CCCreateGiveCredits()
	...
	CCP.GiveCredits.Label:SetText( "Money" );
	...
	function CCP.GiveCredits.OK:DoClick()
		...
		if( LocalPlayer():Money() >= val ) then
			...
			GAMEMODE:AddChat( Color( 200, 200, 200, 255 ), "CombineControl.ChatNormal", "You gave " .. CCSelectedEnt:VisibleRPName() .. " $" .. val .. ".", { CB_ALL, CB_OOC } );
			
		else
			
			GAMEMODE:AddChat( Color( 200, 0, 0, 255 ), "CombineControl.ChatNormal", "You don't have that much money!", { CB_ALL, CB_OOC } );
...
function GM:CCCreatePlayerViewer( ent )
	...
	CCP.PlayerViewer.CharacterInfo:SetPos( 220, 34 );
	CCP.PlayerViewer.CharacterInfo:SetSize( 540, 392 );
	...
	CCP.PlayerViewer.CharacterTitleOne:SetSize( 540, 10 );
	...
	CCP.PlayerViewer.CharacterTitleTwo:SetSize( 540, 10 );
