...
local NormalTranslation = {
	["pistol"] = true,
	["melee"] = true,
	["melee2"] = true,
};

hook.Add( "TFA_GetStat", "Cyberpunk.Holster", function( wep, stat, value )

	if( stat == "HoldType" and wep:GetOwner():Holstered() ) then
	
		if( NormalTranslation[value] ) then
	...
	if( stat == "data.ironsights" and wep:GetOwner():Holstered() ) then
...
hook.Add( "TFA_PreCanPrimaryAttack", "Cyberpunk.Holster", function( wep )

	if( wep:GetOwner():Holstered() ) then
