...
GM.PlayerAccessors = {
	...
	{ "CanFireWeapon",		false,	"Bit",		true },
	{ "DescriptionModifier", false, "String", "" },
	{ "SalaryAmount",		true,	"Int", 0, 32 },
	{ "SalaryInterval",		true,	"Int", 0, 32 },
};

for k,v in next, GM.Stats do

	GM.PlayerAccessors[#GM.PlayerAccessors + 1] = {
		v,
		false,
		"Float",
		1
...
function meta:GetSpeeds()
	...
	local w = 90;
	local r = 200 + self:Agility() * 2;
	local j = 170 + self:Agility() * 2;
	local c = 90;
	
	if( self:HasTrait( TRAIT_MULE ) ) then
		
		r = 130 + self:Agility() * 0.7;
	...
	if( self.Augments ) then
		...
		for k,v in next, self.Augments do
			...
			if( metaitem.RunSpeedModifier ) then
			
				r = r + metaitem.RunSpeedModifier;
				
			end
			if( metaitem.JumpHeightModifier ) then
			
				j = j + metaitem.JumpHeightModifier;
	...
	if( self:LastLegShot() > 0 and !self:HasDrug( DRUG_ANTLION ) ) then
		...
		local b = 15 - ( self:Endurance() / 10 * 15 );
		...
		elseif( d < b + 5 ) then
			...
			r = 90 + ( 60 + self:Endurance() * 0.7 ) * perc;
...
function GM:CPName( name, squad, squadid, cid, flag )
	...
	nameformat = string.gsub( nameformat, "$NAME", name );
